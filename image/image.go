...
// Extract finds an image object in a PDF page and returns a Go image
func Extract(pd *pdfread.PdfReaderT, page int) (image.Image, error) {
	pg := pd.Pages()[page-1]
		...
		for _, ref := range xo {
			...
			case "/DCTDecode": // JPEG
				/*
					...
					bpc := pd.Num(dic["/BitsPerComponent"])
				*/
				return jpeg.Decode(bytes.NewBuffer(data))
